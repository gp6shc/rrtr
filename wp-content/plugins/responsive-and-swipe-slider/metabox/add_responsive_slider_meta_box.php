...
function admin_init_metaboxes(){
	add_meta_box( 'rsSlider_meta_box', 'Images', 'show_rsSlider_meta_box','rkampanj','normal','high'); 
	add_meta_box( 'slider_setting', 'Slider Setting Panel', 'slider_meta_setting', 'rkampanj', 'side', 'low' );	
}

// Field Array
$prefix = 'rsSlider_';
$rsSlider_meta_fields = array(
	array(
		'label'	=> 'Name',
...
if(is_admin()) {
	...
	wp_enqueue_script('admin-js', RS_SLIDER_PLUGIN_URL.'/metabox/responsive_meta_box_js.js', array( 'jquery' ));
	wp_enqueue_style('rsSlider', RS_SLIDER_PLUGIN_URL . '/css/rsSlider_admin.css');
...
// The Callback
function show_rsSlider_meta_box() {
	global $rsSlider_meta_fields, $post;
?>
...
<?php	
	...
	echo '<input type="hidden" name="rsSlider_meta_box_nonce" value="'.wp_create_nonce(basename(__FILE__)).'" />';
	
	// Begin the field table and loop
	echo '<table class="form-table">';
	foreach ($rsSlider_meta_fields as $field) {
		...
		echo '<tr>
				...
				switch($field['type']) {
					case 'repeatable':
						echo '<div id="hdnPlgUrl" style="display:none;">'.RS_SLIDER_PLUGIN_URL.'</div>';
						...
						if ($meta) {
							foreach($meta as $row) {
								...
								if(!empty($exp_data[2])){
									...
									//$imgSrcTim = RS_SLIDER_PLUGIN_URL.'/timthumb/timthumb.php?src='.$path_site.''.$fileInfos.'&h=140&w=256&q=100';
									$attachment_id = $exp_data[2];
									$this_img = wp_get_attachment_image_src( $attachment_id , 'thumbnail' );
									$imgSrcTim = $this_img[0];
									
								}else{
									$imgSrcTim = RS_SLIDER_PLUGIN_URL."/css/noimage.png";
								}
								echo '<li class="li_slide_row">
											<span class="sort hndle span_slide_row">
												<img src="'.RS_SLIDER_PLUGIN_URL.'/css/move.png">
											</span>
							...
							echo '<li class="li_slide_row">
											...
											<div class="upload_image_button button_slide_row"  id="upload_image_button">
												<img class="imgTimThumb" src="'.RS_SLIDER_PLUGIN_URL.'/css/noimage.png">
											</div>
											<div class="content_slide_row">
...
// Save the Data
function save_rsSlider_meta($post_id) {
	
	
    global $rsSlider_meta_fields;
	
	// verify nonce
	if (!wp_verify_nonce($_POST['rsSlider_meta_box_nonce'], basename(__FILE__))) 
		return $post_id;
	// check autosave
...
add_action('save_post', 'save_rsSlider_meta');
...
add_action( 'save_post', 'save_rsSlider_meta_meta' );
function save_rsSlider_meta_meta( $post_id )
{
